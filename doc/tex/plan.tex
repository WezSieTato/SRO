...
\subsection[Spotkanie XI - 19 maja 2015]{Spotkanie XI - 19 maja 2015}
...
\par{Praca programistów nad kodem. Dopisanie szczegółów sytuacji awaryjnych do projektu i sposobów ich rozwiązywania. Testowanie kodu. Konfiguracja Dockera.}
...
\subsection[Spotkanie XV - 10 czerwca 2015]{Spotkanie XV - 10 czerwca 2015}
...
\par{Ostatnie spotkanie skupione głównie na prezentacji projektu.}

\subsubsection*[Szczegółowy opis wykonanej pracy]{Szczegółowy opis wykonanej pracy}
\begin{itemize}
\item Przygotowanie materiałów do prezentacji
\item Przećwiczenie prezentacji
\item Omówienie całokształtu pracy nad projektem
\end{itemize}

\section[Podsumowanie pracy nad projektem]{Podsumowanie pracy nad projektem}

\par{Odbyło się tyle samo spotkań co we wstępnym planie, z tą różnicą, że w nieznacznie innych terminach. Prace i zadania wykonywane w trakcie spotkań nieznacznie się różnią od założonych, co jest związane z trudnością w przewidywaniu konkretnych problemów, opóźnień czy błędów, które wystąpiły przy tworzeniu projektu.}

\par{Implementacja projektu w ostatecznej formie na dzień jego oddania nie jest idealnym odzwierciedleniem systemu zawartego w dokumentacji. Choć kod źródłowy był tworzony zgodnie z samą dokumentacją, krok po kroku, wystąpiło po drodze wiele problemów (nie związanych z samą architekturą), które ostatecznie uniemożliwiły w ograniczonym czasie i przy ograniczonych zasobach stworzyć sprawnie działający projekt. W związku z tym zrezygnowaliśmy z części funkcjonalności na rzecz najważniejszych, naszym zdaniem, zagadnień ukazujących meritum tworzenia aplikacji rozproszonych.}
...
\begin{enumerate}
\item Wstępne przedstawienie projektu
\item Zaprezentowanie zestawiania połączenia między warstwami (w określonym, pomyślnym i bezawaryjnym scenariuszu) przy użyciu 3 komputerów i 3 węzłów w każdej z warstw
\begin{enumerate}
\item Zaprezentowanie logowania kolejnych węzłów w danej warstwie
\item Zaprezentowanie działania algorytmu równomiernego rozłożenia klientów między serwery
\end{enumerate}
\item Zaprezentowanie działającego systemu
\item Zaprezentowanie sytuacji awaryjnej
\item Podsumowanie, napotkane problemy, wnioski
\end{enumerate}
